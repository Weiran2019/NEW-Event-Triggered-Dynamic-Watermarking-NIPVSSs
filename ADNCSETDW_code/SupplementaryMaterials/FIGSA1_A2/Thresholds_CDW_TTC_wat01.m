...
%%第1组数据
Data = importdata('CDW_TTC_wat01_hel1.txt');
...
ind2=zeros(1,len);
for i=2:len
    bR=bR+([res1(i)*res1(i),res1(i)*res2(i);res2(i)*res1(i),res2(i)*res2(i)]-Sigma_ro);
    bD=bD+wat(i-1)*[res1(i);res2(i)];
    ind1(i)=norm(bR/(i-1));
    ind2(i)=norm(bD/(i-1));
end
...
le11=xlabel('$i$');
le12=ylabel('$\frac{1}{i}{(\mathcal{R}_{i}-i\mathcal{E}_r^f)}$');
...
%%第2组数据
Data = importdata('CDW_TTC_wat01_hel2.txt');
len=length(Data.data(:,1));
len_vec(2)=len;
pos=Data.data(:,2);
ang=Data.data(:,3);
...
%%第3组数据
Data = importdata('CDW_TTC_wat01_hel3.txt');
len=length(Data.data(:,1));
len_vec(3)=len;
pos=Data.data(:,2);
ang=Data.data(:,3);
...
%%第4组数据
Data = importdata('CDW_TTC_wat01_hel4.txt');
len=length(Data.data(:,1));
len_vec(4)=len;
pos=Data.data(:,2);
ang=Data.data(:,3);
...
plot(gamma(1:500),'+m','LineWidth',2);
le11=xlabel('$k$');
le12=ylabel('$\gamma(k)$');
ylim([-1,2]);
set(gca,'FontSize',Fonts);
...
%%第5组数据
Data = importdata('CDW_TTC_wat01_hel5.txt');
len=length(Data.data(:,1));
len_vec(5)=len;
pos=Data.data(:,2);
ang=Data.data(:,3);
...
%%第6组数据
Data = importdata('CDW_TTC_wat01_hel6.txt');
len=length(Data.data(:,1));
len_vec(6)=len;
pos=Data.data(:,2);
ang=Data.data(:,3);
...
rho2=1;
for k=2:max_len
   thd1(k)=sqrt((1+rho1)*kap1*log((k-1))/(k-1));
...
saveas(fig3,'Thd_rr_CDW_TTC_wat01','pdf');
saveas(fig4,'Thd_rd_CDW_TTC_wat01','pdf');
saveas(fig1,'CDW_TTC_wat01_nor4_x1','pdf');
saveas(fig2,'CDW_TTC_wat01_nor4_x2','pdf');
saveas(fig5,'CDW_TTC_wat01_nor4_ga','pdf');
